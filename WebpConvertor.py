...
def process_files(input_path: Path, output_type: str, export_folder: Path) -> tuple[int, list[Path]]:
    ...
    for source_file in input_path.glob("*"):
        ...
        if source_file.suffix.lower() in ['.jpg', '.jpeg', '.png']:
            print(f"{Fore.CYAN}Found existing image: {source_file.name} (will include in ZIP)")
        ...
        try:
            ...
            print(f"{Fore.RED}Failed to process {source_file.name}:\n   {Fore.YELLOW}{e}")
...
def main():
    ...
    while True:
        ...
        input_folder = input("Enter folder path containing files (without quotes): ").strip('"')
        output_type = input("Target image type for WebP files (JPG/PNG): ").strip().lower()
        ...
        total_converted, existing_files = process_files(input_path, output_type, export_folder)
